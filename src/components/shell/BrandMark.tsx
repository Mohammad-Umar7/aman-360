...
export function BrandMark({ size = 28, className }: { size?: number; className?: string }) {
  // The mark renders several times per page; each instance needs its own gradient id.
  const g = useId();
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" className={className} aria-hidden>
      ...
      <circle cx="20" cy="20" r="17" fill="none" stroke={`url(#${g})`} strokeWidth="2.2" opacity="0.9" />
      <circle cx="20" cy="20" r="10.5" fill="none" stroke={`url(#${g})`} strokeWidth="1.4" opacity="0.55" />
