...
export function Sidebar() {
  ...
  return (
    <aside className="w-[232px] shrink-0 h-full flex flex-col border-r border-line bg-bg-1/80">
      <Link href="/" className="h-14 flex items-center gap-3 px-4 border-b border-line hover:bg-white/[0.03]">
        ...
        <div className="leading-tight">
          ...
          <div className="text-[10.5px] text-ink-3">Command platform</div>
        </div>
      </Link>
        ...
        {NAV.map((n) => {
          ...
          return (
            <Link
              ...
              href={n.href}
              className={cn(
                "flex items-center gap-2.5 h-9 px-2.5 rounded-lg text-[13px] font-medium transition-colors",
            ...
            >
              ...
              {count !== undefined && <span className="num text-[10.5px] px-1.5 h-4.5 rounded-md bg-white/[0.08] text-ink-2 leading-[18px]">{count}</span>}
            </Link>
          );
        })}
      </nav>
      <div className="mt-auto p-3.5 border-t border-line text-[11px] text-ink-3 leading-4 space-y-2">
        <div className="flex items-center gap-2">
          <span className="h-6 w-6 rounded-full bg-gradient-to-br from-teal/40 to-teal/10 border border-white/10 flex items-center justify-center text-[10px] font-semibold text-ink">NH</span>
          <div>
