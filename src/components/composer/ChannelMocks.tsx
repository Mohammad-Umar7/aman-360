  ...
  return (
    <div className="rounded-xl border border-line bg-[#0d1420] overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-line bg-white/[0.03]">
        ...
        <span className="text-[12.5px] font-medium">Automated voice call · IVR</span>
        <span className="ml-auto text-[11px] text-ink-4">{variant.meta}</span>
      </div>
      <div className="px-4 py-3">
        ...
        <div className="mt-3 flex gap-1.5">
          {["1 · Safe", "2 · Help", "9 · Repeat"].map((k) => (
            <span key={k} className="mono rounded-md border border-line bg-white/[0.04] px-2 py-0.5 text-[11px] text-ink-2">
              {k}
            </span>
          ))}
        </div>
...
export function WebNotice({ variant, lang, stale }: { variant: ChannelVariant; lang: Lang; stale?: boolean }) {
  ...
  return (
    <div className="rounded-xl border border-line bg-[#f4f6fa] text-[#111827] overflow-hidden">
      ...
      <div className="p-3">
        {stale ? (
          <div className="rounded-lg border border-[#d1d5db] bg-white px-3 py-2.5 text-[12.5px]">
          ...
          <div dir={rtl ? "rtl" : "ltr"} className={cn("rounded-lg border-l-4 border-[#c8322b] bg-white px-3 py-2.5 shadow-sm", rtl && "font-arabic border-l-0 border-r-4")}>
            ...
            <div className="text-[13.5px] font-semibold mt-1">{rtl ? variant.titleAr : variant.title}</div>
            <p className="text-[12.5px] leading-5 mt-1 text-[#374151]">{rtl ? variant.ar : variant.en}</p>
          </div>
        )}
      </div>
    </div>
...
export function SignageMock({ variant, lang }: { variant: ChannelVariant; lang: Lang }) {
  ...
  return (
    <div className="rounded-xl border border-line bg-[#0d1420] overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-line bg-white/[0.03]">
        <BrandMark size={14} />
        <span className="text-[12.5px] font-medium">Digital signage · VMS-07</span>
        <span className="ml-auto text-[11px] text-ink-4">{variant.meta}</span>
      </div>
      <div className="p-4">
