    ...
    >
        ...
        {[winner, ...losers].map((c) => {
          ...
          return (
            <div key={c.id} className={cn("rounded-lg border px-3 py-2.5", win ? "border-teal/40 bg-teal/[0.06]" : "border-alert/30 bg-alert/[0.05]")}>
              ...
              <div className="text-[12px] font-medium truncate">{src.name.split(" — ")[0]}</div>
              <div className="text-[18px] font-semibold num tracking-tight mt-0.5">{c.value.toUpperCase()}</div>
              <div className="mono text-[10.5px] text-ink-3">observed {c.observedAt}</div>
              {c.stale && <div className="text-[10.5px] text-warn mt-0.5">stale content</div>}
            </div>
          );
        })}
      </div>
      ...
      <div className="mt-3">
        ...
        <ol className="flex items-stretch gap-1.5">
          {ranks.map((s, i) => {
            ...
            return (
              <li key={s.id} className={cn("flex-1 rounded-md border px-2 py-1.5 text-[11px] leading-4 relative", isWinner ? "border-teal/40 bg-teal/[0.08] text-ink" : isLoser ? "border-alert/30 bg-alert/[0.05] text-ink-2" : "border-line text-ink-3")}>
                <div className="num text-[10px] text-ink-4">#{i + 1} · rank {s.authorityRank}</div>
      ...
      <div className="mt-3">
        ...
        <ol className="space-y-1.5">
          {ctr.rationale.map((r, i) => (
            <li key={i} className="flex gap-2 text-[12px] text-ink-2 leading-4.5">
              <span className="mono text-[10.5px] text-teal-2 mt-0.5 shrink-0">{String(i + 1).padStart(2, "0")}</span>
              {r}
            </li>
          ))}
        </ol>
        <div className="mt-2 flex items-start gap-2 text-[12px] text-ink-2 rounded-md bg-white/[0.03] border border-line px-2.5 py-2">
      ...
      <div className="mt-3 rounded-lg border border-violet/25 bg-violet/[0.05] px-3 py-2.5">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-[12px] font-semibold text-violet-2">Plain-language brief for the operator</span>
          <LayerTag layer="ai" />
        </div>
        {lang === "ar" ? <Arabic className="text-[13px] text-ink leading-6">{explanation.textAr}</Arabic> : <p className="text-[12.5px] text-ink leading-5">{explanation.text}</p>}
        <div className="text-[10.5px] text-ink-4 mt-1.5">Explanation only — the decision above was made by the deterministic layer.</div>
      </div>
    </Panel>
  );
}
